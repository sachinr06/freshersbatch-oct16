...
public class App 
{
    ...
    /*	OrderBean bean = (OrderBean)iocContainer.getBean("order");
    	...
    	*/
    	int total=iocContainer.getBeansOfType(OrderBean.class).entrySet().stream().map(Map.Entry::getValue).map(OrderBean::getItem).map(ItemBean::getPrice).mapToInt(Integer::intValue).sum();
